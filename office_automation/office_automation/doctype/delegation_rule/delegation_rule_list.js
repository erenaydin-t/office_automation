...
//
// Applies the Office Automation template look to the Delegation Rule LIST page.
// Self-contained: defines its own style injector (the list script loads without
// the form script), guarded by element id so it never double-injects.
...
.oa-form .form-section .section-head {
	font-weight: 600; color: var(--heading-color, #1a1c22); letter-spacing: -.01em;
}
...
`;
	...
	frappe.listview_settings["Delegation Rule"] = {
		onload(listview) {
			injectStyle();

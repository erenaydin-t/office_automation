...
//
// Applies the Office Automation template look to the Delegation Rule FORM only.
// Self-contained: it injects its own scoped stylesheet (guarded by id) and adds
// the `oa-form` class to this form's page — nothing else in ERPNext is touched.

frappe.ui.form.on("Delegation Rule", {
	refresh(frm) {
...
.oa-form .form-section .section-head {
	font-weight: 600;
	color: var(--heading-color, #1a1c22);
	letter-spacing: -.01em;
}
...
.oa-form .page-title .title-text, .oa-list .page-title .title-text {
